...
// Datos de usuarios (se actualizarán dinámicamente)
...
const stats = computed((): UsuariosStats => {
  ...
  const activos = usuarios.value.filter((u: Usuario) => u.activo).length;
  const responsables = usuarios.value.filter((u: Usuario) => u.rol === 'RESPONSABLE').length;

  // Nuevos usuarios este mes
  const thisMonth = new Date();
  thisMonth.setDate(1);
  const nuevos_mes = usuarios.value.filter((u: Usuario) =>
