        ...
        <CardContent class="p-6">
          ...
          <div class="mb-4">
            <img
              ...
              @error="(e) => { const img = e.target as HTMLImageElement; if (img) img.src = '/images/default-course.svg' }"
              loading="lazy"
            />
          </div>
...
const openCreateModal = () => {
  ...
  window.location.href = '/responsable/cursos/create'
}

const editCurso = (curso: Curso) => {
  // Redirigir a la página de edición del curso
  window.location.href = `/responsable/cursos/${curso.id}/edit`
}

const viewCurso = (curso: Curso) => {
  // Redirigir a la página de vista del curso
  window.location.href = `/responsable/cursos/${curso.id}`
}

const toggleCursoActivo = async (curso: Curso) => {
  if (processing.value) return

  try {
    processing.value = true

    const response = await fetch(`/responsable/cursos/${curso.id}/toggle-activo`, {
      method: 'POST',
      headers: {
        'Accept': 'application/json',
    ...
    if (response.ok) {
      ...
      showNotification(
        `Curso ${curso.activo ? 'activado' : 'desactivado'} exitosamente`,
        'success'
      )
    } else {
      const errorData = await response.json().catch(() => ({}))
      showNotification(
        errorData.message || `Error al ${curso.activo ? 'desactivar' : 'activar'} el curso`,
        'error'
      )
    }
  } catch (error) {
